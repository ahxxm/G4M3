...
class NoteAction:
    ...
    def list_note(self, recv=True):
        title = '== PCTF GuestBook =='
        if recv:
            self.io.sendlineafter('Your choice: ', '1')
        else:
            self.io.sendline('1')
        out = self.io.recvuntil(title) 
        pos = out.find(title)
        return out[:pos]
    ...
    def new_note(self, content, recv=True):
        if recv:
            ...
            self.io.sendline('2')
        self.io.sendlineafter('Length of new post: ', str(len(content)))
        self.io.sendafter('Enter your post: ', content)
    ...
    def edit_note(self, idx, content, recv=True):
        if recv:
            ...
            self.io.sendline('3')
        self.io.sendlineafter('Post number: ', str(idx))
        self.io.sendlineafter('Length of post: ', str(len(content)))
        self.io.sendafter('Enter your post: ', content)
